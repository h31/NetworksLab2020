...
func (p Packet) AsString() string {
	...
	for p.options[i] != OptionEnd {
		...
		if t {
			res += MessageNames[p.options[i+1]] + "\n"
		} else {
			for j := 0; j < size; j++ {
...
func MakeOffer(discover Packet, ifi net.Interface, ip []byte) (p Packet, error string) {
	discover.Op = 2
	copy(discover.Yiaddr[:], ip)

...
class Server:
    ...
    def run(self):
        ...
        while self.connected:
            ...
            print(packet_received[0])
            dhcp_message.convert_from_bytes(packet_received[0])  # get a proper format
            if packet_received[0][0] == b'\x01':  # message from the client
...
def process_message(self, msg):
    ...
    elif msg.options[53] == dhcp_messages_types[3] and msg.options[54] == server_address:  # request received
        print('Request accepted')
        ack = self.create_ack(msg)
        if msg.cia_addr != '0.0.0.0':
    ...
    elif msg.options[53] == dhcp_messages_types[7] and msg.options[54] == server_address:  # release received
